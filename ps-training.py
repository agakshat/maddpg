...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        actornetwork1 = ActorNetwork(sess,observation_dim[0],action_dim[0],float(args['actor_lr']),float(args['tau']))
        actornetwork2 = ActorNetwork(sess,observation_dim[n-1],action_dim[n-1],float(args['actor_lr']),float(args['tau']))
        criticnetwork1 = CriticNetwork(sess,n,observation_dim[0],total_action_dim,float(args['critic_lr']),float(args['tau']),float(args['gamma']))
        criticnetwork2 = CriticNetwork(sess,n,observation_dim[n-1],total_action_dim,float(args['critic_lr']),float(args['tau']),float(args['gamma']))
        for i in range(n-1):
